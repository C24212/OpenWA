...
| text     | string | No       | non-empty   | Visible label; resolved from the stored prompt when omitted, else `buttonId` |
...
> **Not a native UI tap.** The gateway relays a `buttonsResponseMessage` /
> `templateButtonReplyMessage` / `listResponseMessage` quoted to the stored prompt. WhatsApp may
> reject or treat it differently from a phone tap. The prompt must already be in the Baileys message
> store (received while the session was live). URL/call CTA buttons cannot be clicked this way —
> only quick-reply style choices and list rows.
