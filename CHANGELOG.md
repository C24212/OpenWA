...
- A plugin's declared ingress ack can no longer write the response headers that decide how a browser treats its own reflected body, set a cookie on the gateway's origin, or claim a transfer or content encoding the ack does not actually use, which made the response undecodable and sent the provider back to retry a delivery already queued. The declared content type still applies, through the allowlist that already governed it.
- An ingress route whose manifest declares a non-string ack body or header value is refused at install and at boot, naming the field, instead of loading and then answering every delivery with that part of the ack silently missing.
...
- A send that quotes a message stores the quoted text, whatever the send is. Only replies did: an image, video, document, location, contact, poll or text sent with `quotedMessageId` stored the quoted id with an empty body, and the dashboard drew a blank quote box above it.
...
- A whatsapp-web.js session that comes up without its page-side call hook says so in the log instead of silently never reporting an incoming call. The library installs the hook only when the page exposes the call collection it expects, so a WhatsApp Web build that moves that collection leaves calls undetected while messages keep working. The check is skipped on an install whose whatsapp-web.js patches were not applied, where a session can reach ready before the hook is installed and the warning would be false ([#1655](https://github.com/rmyndharis/OpenWA/issues/1655)).
