...
- A WebSocket client that emits without an ack callback receives command replies again. The gateway answered by returning a frame, which Socket.IO delivers through the ack callback and nowhere else, so a client written to the documented `message` event saw no subscribe confirmation, no pong, and none of the refusals, including the session-scope denial. Replies now go out on `message` as well as through the ack.
- A Baileys message the account sent from its phone is no longer lost when the message store cannot be read. The repeat-delivery check threw on a locked database or an unparseable row and the message was dropped with it, and WhatsApp does not re-deliver one it has already acked; the check now fails open. Two deliveries of one such message landing before the store commits are also reported once rather than twice.
- Baileys `editMessage` returns the edited message's id instead of the edit envelope's, which named a message no route could address and disagreed with the whatsapp-web.js engine.
- The Baileys address book survives a partial loss, not only an empty store. During the initial sync WhatsApp folds saved contacts into the history batch, where their names are stripped as chat titles, so the store kept plenty of contacts with the saved ones missing and the previous repair, which triggered only on an empty store, never ran. The app-state snapshot is now re-pulled once per process.
- Saved Baileys contacts are no longer evicted by peers seen once in a group or a broadcast. Both populations shared one capped map, so a busy session answered `GET /api/sessions/:sessionId/contacts` with fewer and fewer of the contacts the account actually saved.
...
- A session proxy whose credentials contain a bare `%` is refused when it is set, naming the escape, instead of failing every later start and every proxied URL fetch with an opaque `URI malformed`.
